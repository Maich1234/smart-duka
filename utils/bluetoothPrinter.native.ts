...
export const printBytes = async (printer: SavedPrinter, bytes: Uint8Array): Promise<void> => {
  ...
  if (!(await ensurePrinterPermissions())) {
    throw new PrinterError('permission_denied', 'Smart Duka needs Bluetooth permission to reach the printer.');
  }
...
const toPrinterError = (error: unknown, fallback: string): PrinterError => {
  ...
  switch (code) {
    ...
    case 'ERR_BT_PERMISSION':
      return new PrinterError('permission_denied', 'Smart Duka needs Bluetooth permission to find printers.', error);
